...
export class Download implements OnInit, OnDestroy {
  ...
  private connectStream(requestId: string, token: string): void {
    ...
    this.eventSource.onmessage = (event: MessageEvent) => {
      ...
      try {
        ...
        // console.log('[Download] SSE event received:', data);
      } catch {
        console.error('[Download] Failed to parse SSE event data:', event.data);
        return;
      }
    ...
    this.eventSource.onerror = () => {
      ...
      console.error('[Download] SSE connection lost for requestId:', requestId);
  ...
  private handleEvent(data: Record<string, any>): void {
    ...
    switch (data['status']) {
      ...
      case 'completed': {
        ...
        // console.log('[Download] Completed event — server response:', data);

        if (!rawUrl) {
          console.error('[Download] Completed event missing download URL. Full response:', data);
          this.markDone();
      ...
      case 'cloud_success':
        // console.log('[Download] Cloud upload success — server response:', data);
      ...
      case 'cloud_error':
        console.error('[Download] Cloud upload error — server response:', data);
      ...
      case 'error':
        console.error('[Download] Error event from server:', data['message'], '| Full response:', data);
  ...
  copyDriveLink(): void {
    ...
    navigator.clipboard.writeText(url).then(() => {
      ...
      console.error('[Download] Failed to copy Drive link to clipboard:', err);
    });
  }
